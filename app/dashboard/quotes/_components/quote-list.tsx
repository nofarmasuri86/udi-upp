...
const STATUS_MAP: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  draft:    { label: "טיוטה",   color: "text-white/40 bg-white/[0.06] border-white/10",       icon: <Clock className="h-3 w-3" /> },
  sent:     { label: "נשלחה",   color: "text-blue-300 bg-blue-500/10 border-blue-500/20",      icon: <Send className="h-3 w-3" /> },
  ...
  return (
    <div className="flex flex-col gap-2">
      {quotes.map(q => {
        ...
        return (
          <div key={q.id} className="rounded-2xl bg-white/[0.03] border border-white/[0.07] px-4 py-3 flex items-center gap-3">
            {/* Status badge */}
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-xl border text-xs font-bold shrink-0 ${st.color}`}>
              {st.icon}
              {st.label}
              {isExpired && <span className="text-amber-400 mr-1">· פגה</span>}
            </div>

            {/* Info */}
            <div className="flex-1 min-w-0">
              <p className="text-white text-sm font-bold truncate">{q.title}</p>
            ...
            <div className="flex items-center gap-1 shrink-0">
              {q.status === "draft" && (
                <button onClick={() => handleStatus(q.id, "sent")} disabled={isPending}
                  className="text-[10px] font-bold px-2 py-1 rounded-lg bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 transition-colors">
                  סמן נשלח
                </button>
              )}
              {q.status === "sent" && (
                <>
                  <button onClick={() => handleStatus(q.id, "accepted")} disabled={isPending}
                    className="text-[10px] font-bold px-2 py-1 rounded-lg bg-green-500/20 text-green-300 hover:bg-green-500/30 transition-colors">
                    אושר ✓
                  </button>
                  <button onClick={() => handleStatus(q.id, "rejected")} disabled={isPending}
                    className="text-[10px] font-bold px-2 py-1 rounded-lg bg-red-500/20 text-red-300 hover:bg-red-500/30 transition-colors">
                    נדחה ✗
                  </button>
                </>
              )}
              <button onClick={() => handleDelete(q.id)} disabled={isPending}
                className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${
                  confirmDelete === q.id ? "bg-red-500/30 text-red-300" : "bg-white/[0.05] text-white/30 hover:bg-red-500/20 hover:text-red-400"
                }`}>
